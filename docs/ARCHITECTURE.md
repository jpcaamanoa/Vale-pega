...
Esta tabla, ordenada por fecha, **es** la evolución cuantitativa del paciente. **Corrección
post-Fase 13**: la línea original de esta sección planeaba consultar esta tabla desde Recharts —
la Fase 13 decidió explícitamente no instalar ninguna librería de gráficos y mostrar la evolución
como una tabla simple (mismo criterio conservador de dependencias ya aplicado en Fase 6.1,
`docs/geographic-stats.md`). Ver `docs/assessments.md` sección 10.
...
Implementado en la Fase 16 — ver `docs/documents.md` para el diseño completo. Resumen, corrigiendo
el diseño preliminar que este documento describía antes de esa fase:

- **Ubicación**: directorio de datos de la app resuelto vía la API `path` de Tauri
  (`app_data_dir`), `.../CuadernoClinico/vault/files/<2-hex>/<uuid>.enc` — con sharding de 2
  caracteres hexadecimales, no un único directorio plano.
...
- **Cifrado por archivo**: AES-256-GCM (crate `aes-gcm`) con nonce aleatorio por archivo, usando
  una **DEK aleatoria independiente por archivo** (nunca la DEK del vault directamente) —
  envuelta (envelope encryption) con una clave derivada de la DEK del vault vía
  `security::VaultSession::wrap_file_key`/`unwrap_file_key`. Formato de archivo versionado
  (`format_version = 1`, header con magic + nonce, sin metadata clínica).
- **Relación con pacientes/procesos/sesiones**: FK opcionales en `documents.patient_id` /
  `documents.episode_id` / `documents.session_id`.
- **Eliminación**: soft delete (`deleted_at`) únicamente — el ciphertext físico nunca se borra
  como consecuencia de archivar, permanece disponible para restaurar. No existe hard delete/purge
  en esta fase. Un futuro borrado físico definitivo no puede garantizar borrado forense
  irreversible en discos SSD modernos (wear-leveling).
- **Backup/restauración**: se copian los blobs ya cifrados tal cual (rápido, sin re-cifrar);
  `create_backup` rechaza el respaldo si algún ciphertext referenciado no existe físicamente.
- **Exportación**: "Exportar copia" es una acción explícita y consciente de la usuaria, distinta
  de "Abrir" — descifra en memoria y escribe el resultado en texto plano en el destino que ella
  eligió, con advertencia explícita en la UI. Nunca ocurre automáticamente.
...
> **Corrección de la Fase 10 (5 de septiembre de 2026)** — la Fase 10 implementó Backup/Restore
> real (`docs/backup-restore.md`) y, sobre esa base, corrige dos puntos especulativos de esta
> sección y de la sección 12.6, escritos antes de que existiera código:
>
> - **"Checkpoint de WAL primero"** no aplica: el modo WAL sigue sin activarse (ver
>   `docs/db-schema.md`), y el mecanismo real de snapshot consistente es `VACUUM INTO` sobre la
>   conexión ya desbloqueada — no depende de WAL ni de ningún checkpoint.
> - **"Automático: opcional, configurable (frecuencia, carpeta destino)"** y la "rotación de
>   backups automáticos, conservando los últimos 7" de la sección 12.6 quedan **explícitamente
>   descartadas para esta fase**: la decisión de producto vigente, dictada expresamente al
>   encargar la Fase 10, es Backup y Restore **manuales únicamente** — sin programación, sin
>   frecuencia configurable, sin rotación automática. Un backup automático/programado queda como
>   posible fase futura (10.1), sujeta a su propia aprobación — no se implementó como parte de
>   esta fase ni se asume decidido.
>
> La carpeta `documents/` cifrada, mencionada arriba, sigue sin existir como funcionalidad
> (Documentos clínicos no está implementado) — el formato del contenedor de Fase 10 la reserva
> como entrada opcional para cuando exista, pero ningún backup de hoy la incluye.
...
| **8** — Continuidad entre sesiones (preparación + tareas terapéuticas) | ✅ Completada | Dos entidades operativas nuevas sobre migración `V3` (aditiva, `SCHEMA_V1`/`V2` intactos): `patient_prep_notes` ("qué preparar para la próxima sesión", tres estados `pendiente`/`abordado`/`descartado`, editable solo mientras `pendiente`, **sin `deleted_at`** porque el propio `status` ya representa el ciclo de vida completo, `origin_session_id` siempre opcional — una preparación nunca depende de una cita futura agendada) y `therapy_tasks` (tareas entre sesiones, cinco estados —`pendiente`/`parcial`/`realizada`/`no_realizada` más `descartada`, agregado con justificación explícita: cubre una tarea irrelevante *antes* de ser revisada en cualquier sesión, distinta de `no_realizada`—, **con `deleted_at`** porque archivar es un acto administrativo separado de sus estados clínicos, `goal_id` opcional con `ON DELETE SET NULL`). `session_notes.next_focus`/`homework_tasks` (Fase 4) no se tocan ni se reinterpretan — siguen siendo texto histórico dentro de una nota inmutable una vez cerrada; las tablas nuevas son complementarias, nunca un reemplazo. Misma regla de integridad no negociable que Objetivos/Pagos, extendida a un segundo caso: el servicio reverifica explícitamente `session.patient_id`/`goal.patient_id` contra el paciente de la tarea o preparación, tanto al crear como al editar, nunca confiando en el `patientId` recibido desde React. **Sin `episode_id` especulativo**: se evaluó explícitamente separar paciente↔episodio/proceso terapéutico a futuro y se decidió no anticipar ninguna columna — ambas tablas quedan ancladas a `patient_id`, en la misma posición que el resto del esquema, documentado en detalle en `docs/session-continuity.md` en vez de implementado. `SessionContinuityPanel` (componen `PrepNotesSection`/`TherapyTasksSection`) visible tanto en la pestaña "Sesiones" de la ficha del paciente como dentro de `SessionDetailScreen` (que solo recibió un import y una línea de render, sin agrandar su lógica). Dashboard: "Pendientes" pasa a mostrar el conteo real de tareas clínicas pendientes (`COUNT(*)` en el backend) sin fingir que Documentos existe; "Sesiones del mes" (placeholder pendiente desde la Fase 7) se cerró como agregado trivial adicional, mismo patrón SQL que "Ingresos del mes". Paciente archivado: creación de tareas/preparaciones nuevas bloqueada en el backend; las existentes siguen editables. 423/423 tests Rust en verde (355 previos sin cambios + 68 nuevos: 16 en `services::patient_prep_notes`, 7 en `repositories::patient_prep_notes`, 23 en `services::therapy_tasks`, 15 en `repositories::therapy_tasks`, 5 en `db::migrations` para `V3`, 2 en `repositories::sessions` para el nuevo conteo mensual), `cargo clippy` sin advertencias, build/lint frontend limpios (19 warnings, exactamente las mismas dos categorías preexistentes, ninguno originado en un archivo nuevo de esta fase — verificado explícitamente), cero dependencias nuevas, más verificación manual completa sobre la aplicación real compilada (crear preparación y tarea desde la ficha del paciente, verlas dentro de una sesión real, marcar tarea "Parcial" y preparación "Abordado" desde dentro de la sesión confirmando que salen de "pendientes" sin perderse del historial, tarea vinculada a un objetivo del mismo paciente con el título correcto mostrado, paciente archivado bloqueando solo creación nueva, persistencia a través de un cierre/reapertura completos del proceso, y una segunda pasada de regresión funcional de Pacientes/Objetivos/Agenda/Dashboard) sobre un **vault de prueba desechable** (conservado bajo nombre de respaldo, nunca eliminado; vault real restaurado exactamente como estaba), y auditoría de privacidad con un marcador ficticio (`XYZFASE8CONTINUIDAD`) confirmando cero fugas fuera del vault cifrado, incluyendo confirmación de que ningún archivo nuevo de backend hace logging de contenido clínico. Detalle completo en `docs/session-continuity.md` |
| **9** — Episodios terapéuticos mínimos (`treatment_episodes`) | ✅ Completada | Resuelve el problema estructural "paciente ≠ proceso" identificado en la auditoría post Fase 8: nueva entidad `treatment_episodes` (en UI, siempre "Proceso"/"Proceso terapéutico" — nunca "episodio") sobre migración `V4` (aditiva, `SCHEMA_V1`/`V2`/`V3` intactos). Modelo mínimo (`id, patient_id, started_at, status, created_at, updated_at, deleted_at`) — `reason_for_end`/`closure_summary`/`recommendations`/`outcome`/etc. quedan explícitamente para la futura Fase 10 (Cierre/Alta); no se creó `episode_closures`. **Un solo proceso activo por paciente**, garantizado en dos capas independientes (servicio + índice único parcial `idx_treatment_episodes_one_active_per_patient`, mismo criterio que `idx_session_notes_current` de la Fase 4). `'cerrado'` es legal en el `CHECK` del esquema (preparación para Fase 10) pero **inalcanzable** desde el servicio (`ClosureNotImplemented`) y desde la UI (sin botón "Cerrar") — Opción A de la aprobación. `sessions.episode_id`/`therapeutic_goals.episode_id` nullable, con `patient_id` siempre obligatorio — una sesión puede seguir existiendo sin proceso. Regla de integridad reutilizada (`check_episode_assignable`): un proceso solo se puede asignar si pertenece al mismo paciente, no está archivado y no está cerrado — igual criterio no negociable de Fases 5/7/8 de no confiar en IDs recibidos desde React. `episode_clinical_profile` nueva (1:1 con el proceso, mismo patrón que `patient_clinical_profile`), con exactamente `presenting_problem`/`primary_diagnosis_code`/`diagnosis_notes` — `risk_flags`/`relevant_medical_notes` permanecen longitudinales en `patient_clinical_profile`, sin tocar esa tabla ni diseñar ninguna taxonomía de riesgo. Migración legacy: como máximo un proceso automático por paciente con actividad clínica real (sesión, objetivo o antecedentes — nunca por pacientes solo con pagos/tareas/preparaciones), estado derivado únicamente de `patients.status` (`'alta'`→`'cerrado'`), sin ninguna heurística de fecha, con `started_at` derivado de la primera sesión → `intake_date` → `created_at`, e ID determinístico (`'legacy-' || patient_id`) para auditabilidad. `patients.intake_date`/`patients.status` sin cambios de semántica ni de valores del `CHECK`; `appointments` no recibe `episode_id`; `src-tauri/src/calendar/*` no se tocó (verificado por `grep`). 491/491 tests Rust en verde (423 previos sin cambios + 68 nuevos: en `repositories`/`services` de `treatment_episodes` y `episode_clinical_profile`, 6 en `db::migrations` para `V4`, más extensiones en `services::sessions`/`services::goals`), `cargo clippy` sin advertencias, build/lint frontend limpios (20 warnings: 19 preexistentes + 1 nuevo de la misma categoría ya presente en el resto del código — sin categoría nueva), cero dependencias nuevas, más verificación manual completa sobre la aplicación real compilada cubriendo los 9 casos de la aprobación (nuevo proceso con sesión y objetivo vinculados, sesión sin proceso, un solo proceso activo por paciente, pausar/reactivar, paciente archivado, persistencia a través de bloqueo/desbloqueo y de un cierre/reapertura completos del proceso) y una segunda pasada de regresión funcional de Pacientes/Antecedentes/Sesiones/Objetivos/Pagos/Agenda/Estadísticas/Dashboard sobre un **vault de prueba desechable** (conservado bajo nombre de respaldo, nunca eliminado; vault real restaurado exactamente como estaba), y auditoría de privacidad con un marcador ficticio (`XYZFASE9EPISODIOSMARKER`) confirmando cero fugas fuera del vault cifrado. Detalle completo en `docs/treatment-episodes.md` |
| **10** — Backup y restauración segura | ✅ Completada | Backup y Restore **manuales únicamente** (sin programación, sin rotación, sin nube) — objetivo: que si el computador, disco o vault principal se pierde o se daña, exista un respaldo real y verificado del que recuperarse. Contenedor nuevo `.cclinbackup` (ZIP sin compresión, formato `v1`) con `manifest.json` (minimizado por el propio tipo Rust — sin ningún campo capaz de llevar un dato clínico), `vault.db` y `vault.meta.json`; `documents/` reservado para una fase futura, no implementado. Snapshot consistente vía `VACUUM INTO` sobre la conexión ya desbloqueada (nunca `fs::copy` con conexión viva) — verificado empíricamente que el resultado sigue siendo un SQLCipher válido con la misma clave, y que **no fue necesario activar el modo WAL** para lograrlo (sigue diferido, ahora solo por una posible razón de rendimiento futuro — ver corrección en `docs/db-schema.md`). Restore es reemplazo, nunca fusión: extrae y valida todo (manifest, hashes SHA-256, credencial contra el propio backup, compatibilidad de esquema sin downgrade, `foreign_key_check`) en un directorio de staging desechable, y solo si **todas** las validaciones pasan promueve el staging mediante un swap atómico de dos `rename`, conservando el vault anterior en `vault-rescue` hasta una revalidación posterior — un backup inválido en cualquier punto nunca toca un solo byte del vault activo. Recuperación ante un crash a mitad del swap basada únicamente en presencia/ausencia de directorios en disco (`run_startup_recovery`, sin marcador nuevo), verificada con tests dedicados para los tres casos posibles. Reutiliza sin duplicar `security::unlock_vault`/`recover_access`/`VaultSession` (reexportes aditivos); `session.lock()` + nuevo método `refresh_from_disk()` sincronizan el estado de la sesión activa tras un restore. Backup nunca incluye tokens de Google (viven solo en el keychain del SO); `src-tauri/src/calendar/*` no se tocó. Dos dependencias nuevas pre-aprobadas: `tauri-plugin-dialog` (diálogos nativos de archivo) y el crate `zip`. Sin migración de esquema SQL (`schema_version` sigue en `4`). 523/523 tests Rust en verde (491 previos sin cambios + 32 nuevos en `backup::service`), `cargo clippy` sin advertencias, build/lint frontend limpios (sin warnings nuevos), más verificación manual completa sobre la aplicación real compilada: backup real de un paciente ficticio con marcador (`XYZFASE10BACKUP`), modificación posterior, restore completo con verificación exacta de que el estado vuelve al momento del backup (incluyendo que el paciente creado después desaparece), persistencia a través de bloqueo/desbloqueo y de un cierre/reapertura completos del proceso, y una **prueba de corrupción real** (copia de un backup corrompida deliberadamente, restauración rechazada con mensaje claro, vault activo confirmado byte-a-byte idéntico antes/después vía SHA-256) sobre un **vault de prueba desechable** (conservado bajo nombre de respaldo, nunca eliminado; vault real restaurado exactamente como estaba), y auditoría de privacidad con marcador ficticio confirmando cero fugas fuera del vault cifrado, incluyendo el propio archivo de backup. Corrige dos afirmaciones especulativas de la sección 9 y 12.6 escritas antes de que existiera código (backup automático/rotación de 7 copias, checkpoint de WAL) — ver nota de corrección en la sección 9. Detalle completo en `docs/backup-restore.md` |
| **11** — Cierre/alta estructurado de procesos terapéuticos (`episode_closures`) | ✅ Completada | Construye el único camino real hacia `treatment_episodes.status = 'cerrado'`, legal en el esquema desde la Fase 9 pero inalcanzable hasta ahora (`ClosureNotImplemented`). Tabla nueva `episode_closures` (migración `V5`, aditiva, `SCHEMA_V1`–`V4` intactos), 1:1-con-historial vía índice único parcial `idx_episode_closures_active` (`WHERE reverted_at IS NULL`) — mismo criterio que `idx_treatment_episodes_one_active_per_patient` (Fase 9) e `idx_session_notes_current` (Fase 4): como máximo un cierre **vigente** por proceso, historial de cierres anulados conservado para siempre. **Inmutable tras crearse** — decisión explícita de la aprobación (frente a editable/versionado): `repositories::episode_closures` no tiene ninguna función de `update` de contenido; corregir un error es `revert_closure` (anula, preserva el original) seguido de un `close_episode` nuevo. Taxonomía de motivo fija de 6 categorías (`alta`/`cierre_acordado`/`interrupcion`/`derivacion`/`decision_profesional`/`otro`, este último exige detalle) y `outcome` (resultado) como campo **independiente** del motivo — verificado manualmente con `derivacion` + `parcialmente_logrados` simultáneos. Sesiones futuras del proceso: resolución manual explícita y obligatoria en el backend (`session_resolutions` debe cubrir exactamente las sesiones `programada` con `session_date > date('now')`, ni de más ni de menos — `PendingSessionResolutionRequired`/`UnknownSessionInResolution`); tareas/preparaciones pendientes generan solo una advertencia informativa, nunca bloquean ni se modifican. Reapertura exige motivo y **pregunta siempre explícitamente** el estado destino (`activo`/`pausado`, nunca asume) y reutiliza sin duplicar la regla `AnotherEpisodeActive` de Fase 9: el caso crítico exigido por la aprobación — Proceso A cerrado, Proceso B creado y activo (reingreso), reabrir A como activo — queda rechazado tanto a nivel de servicio como verificado manualmente en la UI, mientras que reabrir A como pausado en ese mismo escenario es válido. El reingreso en sí no requirió código nuevo: `create_episode` ya excluía procesos `'cerrado'` de su verificación de proceso activo desde la Fase 9. `patients.status` no se toca en ningún punto de este flujo — sigue sin ser la fuente de verdad. `backup::service::current_app_schema_version()` calcula la versión dinámicamente, por lo que `V5` no exigió ningún cambio de diseño en Backup/Restore (solo dos literales de test). 570/570 tests Rust en verde (523 previos sin cambios + 47 nuevos: repositorio y servicio de `episode_closures`, 6 en `db::migrations` para `V5`, más extensiones en `services::sessions`/`repositories::goals`/`repositories::therapy_tasks`), `cargo clippy --release` sin advertencias, build/lint frontend limpios (21 warnings: 20 preexistentes + 1 nuevo en `ClosureSection.tsx` de la misma categoría ya presente en sus pares — sin categoría nueva), cero dependencias nuevas, más verificación manual completa sobre la aplicación real compilada cubriendo los 8 casos de la aprobación (alta normal con pendientes no bloqueantes, reingreso sin copiar contenido del proceso anterior, corrección de un cierre por error, el conflicto A/B crítico rechazado en vivo, persistencia a través de bloqueo/desbloqueo y de un cierre/reapertura completos del proceso, ciclo completo de Backup/Restore con un proceso cerrado — incluyendo rechazo de un backup corrupto sin tocar el vault activo) sobre un **vault de prueba desechable** (conservado bajo nombre de respaldo, nunca eliminado; vault real restaurado exactamente como estaba), y auditoría de privacidad con marcador ficticio (`XYZFASE11CIERREPRIVACY`) confirmando cero fugas fuera del vault cifrado, incluyendo el respaldo generado (confirmado cifrado, no texto plano) y los directorios temporales de backup/restore (confirmados vacíos tras la operación). **Limitación documentada explícitamente**: no se construyó una pantalla de historial de cierres más allá del cierre vigente — `episodeClosuresApi.listHistory` existe y está cubierta por tests, pero un proceso con más de un cierre en su historia no tiene hoy una vista dedicada para repasar los anteriores desde la interfaz. Detalle completo en `docs/episode-closure.md` |
| **Microcorrección post-Fase 11** — unificar semántica de archivado de pacientes | ✅ Completada | Auditoría de transición detectó que `status='archivado'` era seleccionable manualmente en el formulario normal de paciente sin ejecutar el archivado estructural real (que depende exclusivamente de `deleted_at`, vía `archive_patient`/`restore_patient`) — un paciente podía quedar con `status='archivado'` y `deleted_at=NULL` (seguía activo) o viceversa. Decisión: `deleted_at` es la única fuente de verdad de "archivado"; `'archivado'` permanece en `VALID_STATUSES` y en el `CHECK` de `SCHEMA_V1` sin ningún cambio de esquema (cero migración) — una fila legacy con ese valor sigue siendo legible/editable sin forzar un cambio. `services::patients::validate` ahora recibe el estado actual del paciente y rechaza una *transición nueva* hacia `'archivado'` (`ArchivedStatusIsNotManuallySelectable`), pero nunca bloquea el reenvío sin cambios de un valor legacy ya existente. Frontend: nuevo `SELECTABLE_PATIENT_STATUSES` (activo/inactivo/alta) para el `<select>` editable, con `PATIENT_STATUS_LABELS` intacto para mostrar el valor legacy donde corresponda. 577/577 tests Rust en verde (570 previos + 7 nuevos), `cargo clippy` sin advertencias, build/lint frontend limpios, cero dependencias nuevas. Commit y baseline independientes de la Fase 12. |
| **12** — Plan de Seguridad Clínico Versionado (`safety_plans`) | ✅ Completada | Herramienta de **documentación clínica pura** — nunca un predictor de riesgo, diagnóstico, sistema de emergencia ni vigilancia; la aplicación registra, la profesional decide. Dos tablas nuevas (migración `V6`, aditiva, `SCHEMA_V1`–`V5` intactos): `safety_plans` (por **paciente**, deliberadamente sin `episode_id` — el plan sigue vigente entre procesos, tras un cierre o en un reingreso) con versionado de tres estados `borrador`/`vigente`/`reemplazado` (dos índices únicos parciales: como máximo un `vigente` y como máximo un `borrador` sin confirmar por paciente, mismo patrón que `idx_episode_closures_active` de Fase 11) y `safety_plan_contacts` (tabla hija, `contact_type` en `support_person`/`professional`/`service`, `ON DELETE CASCADE` — solo alcanzable al descartar un borrador nunca confirmado, la única eliminación física permitida en este dominio; `vigente`/`reemplazado` son inmutables e imborrables). "Actualizar plan" nunca es un `UPDATE` sobre el vigente: crea un borrador nuevo precargado con su contenido, y al confirmarlo el vigente anterior se marca `reemplazado` dentro de la misma transacción. IPC minimizado: el historial (`list_safety_plan_history`) devuelve `SafetyPlanSummary` sin ningún campo narrativo ni contactos (garantizado por el propio tipo Rust, no solo por convención) — el contenido completo de una versión histórica solo cruza IPC al abrirla explícitamente (`get_safety_plan_by_id`). Paciente archivado: puede seguir consultando el plan vigente y su historial, pero no puede crear un plan nuevo (autoridad en `services::safety_plans`, no solo en React). Sin ninguna relación de código con `patient_clinical_profile.risk_flags` (dominios conceptualmente distintos, nunca sincronizados en ningún sentido) ni con `treatment_episodes`/`episode_closures` (cerrar un proceso nunca toca el plan). `src-tauri/src/calendar/*` no se tocó (verificado por `grep`: cero referencias a campos del plan). `src-tauri/src/backup/*` no cambió su lógica de producción — `current_app_schema_version()` calcula la versión dinámicamente, por lo que `V6` no exigió ningún cambio de diseño (solo dos literales de test, igual que en la transición V4→V5). 616/616 tests Rust en verde (577 previos tras la microcorrección + 39 nuevos: repositorio y servicio de `safety_plans`, incluyendo el DTO de minimización `SafetyPlanSummary`, más 2 en `db::migrations` para `V6`), `cargo clippy` sin advertencias, build/lint frontend limpios (22 warnings: 21 preexistentes + 1 nuevo en `SafetyPlanTab.tsx` de la misma categoría ya presente en sus pares — sin categoría nueva), cero dependencias nuevas. Nueva pestaña "Plan de seguridad" en la ficha del paciente (`src/features/safety-plan/`), con advertencia clínica sobria visible siempre ("recurso de apoyo... no es un servicio de emergencia"). Detalle completo en `docs/safety-plan.md` |
| **Micro-hardening post-Fase 12** — reforzar versionado y archivado del Plan de Seguridad | ✅ Completada | Dos huecos detectados en el propio informe de cierre de la Fase 12: (1) `update_draft`/`confirm_draft`/`add_contact`/`update_contact`/`delete_contact` no re-comprobaban el archivado del paciente en cada llamada (solo `create_draft` lo hacía) — nuevo helper `require_editable_draft` (compone `require_draft` + una comprobación de archivado) hace la regla autoritativa en el servicio, no solo oculta tras botones de React; `discard_draft` queda deliberadamente exceptuado, decisión de producto confirmada explícitamente (consistente con el criterio ya usado en el resto del dominio de no bloquear retroactivamente edición/descarte de contenido aún no confirmado). (2) "Actualizar plan" copiaba, a mano desde el frontend, solo el contenido narrativo — los contactos del vigente podían desaparecer en silencio al confirmar la nueva versión; nueva función atómica `create_draft_from_current` copia en el backend, dentro de una única transacción, contenido narrativo + `reviewed_at` + todos los contactos (con IDs nuevos, sin compartir fila con la versión anterior). 631/631 tests Rust en verde (616 previos + 15 nuevos), `cargo clippy --release` sin advertencias, build/lint frontend limpios, cero dependencias y cero migración nuevas. Commit y baseline independientes de la Fase 13. |
| **13** — Evaluaciones clínicas / seguimiento psicométrico (`assessment_instruments`/`assessment_administrations`) | ✅ Completada | Primer vertical productivo sobre dos tablas presentes sin usar desde `SCHEMA_V1` (Fase 1.3). Regla de copyright no negociable: la aplicación nunca almacena ni reproduce el contenido real de un instrumento (ítems, opciones, tablas de normas propietarias) — solo metadatos del catálogo (nombre/abreviatura/descripción/categoría, texto libre) y resultados de una administración (puntaje total, subescalas JSON libre, interpretación redactada por la profesional); `raw_responses` (columna legado de `SCHEMA_V1`) queda deliberadamente sin usar, documentado explícitamente en `docs/assessments.md`. Auditoría de esquema previa (obligatoria antes de escribir código, ver `Plan-Fase-13-pendiente-de-aprobacion.md`) encontró dos huecos reales frente a los requisitos de esta fase — sin ellos no se podía cubrir el V1 pedido — resueltos con `SCHEMA_V7` (aditiva, `SCHEMA_V1`–`V6` intactos): `abbreviation`/`category` en `assessment_instruments`, y `episode_id` (vínculo opcional a un proceso terapéutico, mismo patrón que `sessions.episode_id`/`therapeutic_goals.episode_id` de la Fase 9) en `assessment_administrations`, tabla anterior a `treatment_episodes` que nunca se había revisado retroactivamente. Ninguna función de este dominio calcula, normaliza (percentiles/T-scores/Z-scores) ni interpreta automáticamente nada; la evolución longitudinal de un instrumento nunca mezcla instrumentos distintos. Minimización de IPC: `AssessmentAdministrationSummary` (listados) nunca lleva `subscaleScores`/`interpretationText` — solo viajan al abrir una administración concreta, mismo criterio que `SafetyPlanSummary` de la Fase 12. **Decisión conservadora de dependencias**: se evaluó instalar Recharts (planeado originalmente en esta misma sección) y se decidió no hacerlo — la evolución se muestra como tabla simple, mismo criterio ya aplicado en Fase 6.1. 668/668 tests Rust en verde (631 previos del micro-hardening + 37 nuevos: 13 en `repositories::assessments`, 21 en `services::assessments`, 3 en `db::migrations` para `V7`), `cargo clippy --release` sin advertencias, build/lint frontend limpios (23 warnings: 22 preexistentes + 1 nuevo en `AssessmentsTab.tsx` de la misma categoría ya presente en sus pares — sin categoría nueva), cero dependencias nuevas. Nueva pestaña "Evaluaciones" en la ficha del paciente (`src/features/assessments/`), con gestión de catálogo de instrumentos, registro/edición de administraciones y vista de evolución por instrumento. `src-tauri/src/calendar/*` no se tocó. Detalle completo en `docs/assessments.md` |
| **14** — Historial de cierres y hardening longitudinal | ✅ Completada | Microfase que cierra el único hueco declarado explícitamente como limitación conocida en la Fase 11: un proceso reabierto (`revert_closure`) no dejaba ningún rastro visible en la UI de que alguna vez estuvo cerrado, aunque `episodeClosuresApi.listHistory` ya existía y ya tenía tests desde esa fase. `ClosureHistorySection.tsx` (nuevo) se integra en `TreatmentEpisodeDetailScreen.tsx` junto a `ClosureSection` (que no se modificó): muestra únicamente los cierres **anulados** (`revertedAt !== null`), en tarjetas de solo lectura con etiqueta neutra "Cierre anulado" — nunca duplica el cierre vigente, que sigue mostrándose solo en `ClosureSection`. Hardening adicional sin cambio de comportamiento: `today_utc_date` (antes una consulta SQL directa dentro de `services::episode_closures`) se movió a `repositories::episode_closures::today_utc_date`, reforzando la regla de capas "SQL solo en repositorios" — verificado explícitamente con un test que confirma resultado idéntico antes/después. Sin migración de esquema, sin dependencias nuevas. 670/670 tests Rust en verde (668 previos + 2 nuevos), `cargo clippy --release` sin advertencias, build/lint frontend limpios (23 warnings, misma cantidad y categorías que el baseline — sin archivo nuevo de esta fase aportando warnings). Detalle completo en `Informe-de-cierre-Fase-14-Historial-de-Cierres.md` |
| **15** — Formulación clínica textual versionada (`case_formulations`/`formulation_versions`) | ✅ Completada | Segundo vertical productivo sobre tablas presentes sin usar desde `SCHEMA_V1` (Fase 1.3), estrictamente **textual** — canvas visual, nodos/aristas, generación automática y sugerencias diagnósticas quedan explícitamente fuera de alcance. Principio rector: la formulación es una hipótesis clínica de trabajo escrita por la profesional, que se versiona y nunca se sobrescribe, no se genera automáticamente, no se confunde con diagnóstico y no se copia silenciosamente entre procesos. Auditoría de esquema previa (`Plan-Fase-15-pendiente-de-aprobacion.md`) encontró que `case_formulations` solo tenía `patient_id`, sin vínculo al proceso terapéutico — decisión explícita de la Fase 9 de no agregarlo entonces. Aprobada explícitamente la Opción B: `SCHEMA_V8` (aditiva, `SCHEMA_V1`–`V7` intactos) agrega `episode_id` nulable con índice único parcial `idx_case_formulations_one_per_episode` (mismo patrón ya usado 3 veces: `idx_treatment_episodes_one_active_per_patient`/`idx_episode_closures_active`/`idx_safety_plans_one_current_per_patient`) — como máximo una formulación principal por proceso, con múltiples versiones históricas (`formulation_versions`, sin estado de borrador: la vigente es siempre `MAX(version_number)`). A nivel de servicio, `episode_id` es obligatorio (no opcional) pese a ser nulable en el esquema, para no recrear la ambigüedad que esta fase resolvió. Contenido estructurado en 10 secciones (síntesis/predisponentes/precipitantes/perpetuantes/protectores/patrones/hipótesis/focos/plan/observaciones) serializadas por el **frontend** como bloques Markdown dentro de una única columna `summary_text` — el backend nunca interpreta esa estructura, cero columnas SQL nuevas para esto. Historial de versiones de solo lectura desde el primer momento (explícitamente para no repetir la deuda de historial de cierres, Fase 11/14). Reutiliza `check_episode_assignable` (Fase 9) para bloquear una versión nueva si el proceso está cerrado; un reingreso nunca copia la formulación del proceso anterior (verificado con test dedicado); un paciente archivado bloquea contenido nuevo (creación y nuevas versiones) pero preserva lectura completa, mismo criterio que `require_editable_draft` de Plan de Seguridad (Fase 12). Decisiones explícitas de no ampliar alcance: sin integración con Objetivos terapéuticos (`therapeutic_goals.formulation_id` nunca se escribe desde ningún comando existente), sin archivado de formulaciones individuales, y `formulation_nodes`/`formulation_edges` (ya construidas en `SCHEMA_V1`, con triggers de integridad) quedan explícitamente reservadas para una futura Formulación Visual, sin tocarse. Minimización de IPC: `FormulationSummary` nunca lleva `summaryText`. 695/695 tests Rust en verde (670 previos + 25 nuevos: 4 en `db::migrations` para `V8`, 7 en `repositories::formulations`, 14 en `services::formulations`), `cargo clippy --release` sin advertencias, build/lint frontend limpios (24 warnings: 23 preexistentes + 1 nuevo en `FormulationTab.tsx` de la misma categoría ya presente en sus pares — sin categoría nueva), cero dependencias nuevas (sin editor de texto enriquecido, secciones editadas con `<textarea>` simples). Nueva pestaña "Formulación" en la ficha del paciente (`src/features/formulation/`), que distingue "Formulación del proceso actual" de "Formulaciones de otros procesos" sin duplicar la pestaña "Procesos". `src-tauri/src/calendar/*` no se tocó. **Prueba manual GUI bloqueada por una limitación de renderizado de WebKitGTK en este entorno** (confirmada en tres intentos con configuraciones distintas, no relacionada con el código de esta fase) — documentada como pendiente, no inventada, y agregada al Pre-V1 Manual Acceptance Test acumulado. Detalle completo en `docs/formulation.md` |
| **16** — Documentos y adjuntos clínicos cifrados (`documents`) | ✅ Completada | Tercer vertical productivo sobre una tabla presente sin usar desde `SCHEMA_V1` (Fase 1.3). Cada documento tiene una **DEK de 256 bits aleatoria e independiente** (nunca la DEK del vault directamente) cifrada con AES-256-GCM y envuelta (envelope encryption) mediante dos métodos nuevos y mínimos en `security::VaultSession` (`wrap_file_key`/`unwrap_file_key`, única modificación autorizada dentro de `security/*`, en la medida mínima necesaria) — el resto de la lógica de cifrado de archivos vive deliberadamente fuera de `security/*`, en `services::document_crypto`. Formato de archivo versionado (`format_version = 1`: magic + nonce + ciphertext, sin metadata clínica en el header, redundante a propósito con la columna homónima de la fila). Ciphertext guardado con nombre opaco y sharding de 2 hex (`vault/files/<2-hex>/<uuid>.enc`) — el filesystem nunca revela nombre, paciente, categoría, proceso ni sesión; `resolve_within_files_root` valida activamente contra path traversal (8 tests dedicados). `SCHEMA_V9` (reconstrucción completa de tabla, no `ALTER TABLE`, porque SQLite no permite modificar un `CHECK` existente — `SCHEMA_V1` permanece intacto) agrega `episode_id` opcional, `wrapped_file_dek`/`wrap_nonce`/`format_version`, y la categoría `derivacion` al `CHECK` ya existente; `sha256_plaintext` conserva su significado original (hash del contenido descifrado, verificación de integridad al abrir) — se evaluó y descartó reinterpretarla como hash de ciphertext. Importación atómica sin copia plaintext intermedia (cifra en memoria → temporal `.enc.tmp` → `rename` atómico → recién entonces el `INSERT`, verificado leyendo los bytes físicos del resultado), límite de 50 MB por archivo comprobado antes de leer el archivo completo, sin streaming en esta versión. Visualización híbrida por MIME: imágenes se descifran en memoria a un `data:` URL (cero huella en disco); el resto se descifra a un temporal opaco fuera de `vault/files/` y se abre con la aplicación por defecto del sistema (`DocumentTempRegistry`, limpieza al bloquear el vault, al cerrar la aplicación, y barrido de arranque ante un crash). "Exportar copia" es una acción explícita de la usuaria, distinta de "Abrir", con advertencia de que la copia queda fuera del cifrado del vault. Proceso **cerrado** sí puede recibir documentos nuevos (a propósito, distinto de Formulación); paciente **archivado** no puede recibir documentos nuevos pero conserva lectura completa de los existentes. Soft delete únicamente — el ciphertext nunca se borra al archivar un documento; sin hard delete en esta fase. Backup/Restore extendido de forma obligatoria: `create_backup` copia cada ciphertext referenciado (nunca se descifra para respaldar) y rechaza el backup completo si detecta un ciphertext faltante, nunca produce un respaldo incompleto en silencio; `restore_backup` no requirió cambios porque el manifest y el contenedor ya eran genéricos sobre un número arbitrario de archivos; backups antiguos sin `files/` siguen siendo restaurables sin cambios. 766/766 tests Rust en verde (695 previos + 71 nuevos: 5 en `db::migrations` para `V9`, 6 en `security::session` para wrap/unwrap, 22 en `services::document_crypto`, 7 en `services::document_temp`, 10 en `repositories::documents`, 18 en `services::documents`, 9 en `backup::service`), `cargo clippy --release --all-targets` sin advertencias nuevas, build/lint frontend limpios, cero dependencias nuevas (reutiliza `aes-gcm`/`getrandom`/`uuid`/`sha2`/`base64ct`/`tauri-plugin-dialog`/el crate `open`, todos ya presentes). Nueva pestaña "Documentos" funcional en la ficha del paciente (`src/features/documents/`): listar, agregar, abrir/previsualizar, exportar copia, editar metadata, archivar/restaurar activos y archivados — nunca muestra `storage_path`, clave envuelta, nonce, hash ni UUID físico (estructuralmente imposible: `Document`, la fila completa, ni siquiera deriva `Serialize`). `src-tauri/src/calendar/*` no se tocó. **Prueba manual GUI bloqueada por la misma limitación de renderizado de WebKitGTK en este entorno ya documentada en la Fase 15** — no relacionada con el código de esta fase, documentada como pendiente, no inventada. Detalle completo en `docs/documents.md` |
| **17** — Hardening pre-RC Desktop (REG-1/BACKUP-1/MULTI-1/CRYPTO-1) | ✅ Completada | Corrige los cuatro hallazgos clasificados **B (bloqueante antes de RC)** en `Auditoria-Pre-RC-Desktop-post-Fase-16.md`, exactamente según lo aprobado en `Plan-Fase-17-Hardening-Pre-RC-pendiente-de-aprobacion.md` — sin ninguna feature clínica nueva, sin tocar Calendar/Sync/iOS/biometría/updater/hard delete. **REG-1**: `resolve_within_files_root_rejects_an_uppercase_shard` usaba un UUID aleatorio cuyo shard podía caer en dos dígitos decimales, haciendo `.to_uppercase()` un no-op y el test intermitente; se reemplazó por un UUID fijo con shard hexadecimal genuino — 25 corridas aisladas consecutivas en verde, sin tocar `resolve_within_files_root` en sí. **BACKUP-1**: `archive::write_container` creaba el `.cclinbackup` final con `File::create_new` como primer paso, dejando un archivo parcial en `dest_path` si el ZIP fallaba a mitad de camino; ahora construye el contenido en un temporal hermano del destino (mismo directorio — nunca falla por "cross-device link" si el destino es un USB o una unidad de red), lo sincroniza a disco (`sync_all`) y solo lo promueve con `rename` al terminar con éxito. Se investigó `std::fs::hard_link` como primitiva no-clobber verdaderamente atómica y se descartó: falla en FAT32/exFAT, el formato más común en pendrives USB — cambiaba un problema raro por uno común. Queda una ventana de carrera residual, pequeña y documentada sin adornos (`docs/backup-restore.md`), entre una comprobación defensiva y el `rename`; no se afirma una garantía absoluta que técnicamente no existe. **MULTI-1**: se agrega `tauri-plugin-single-instance` 2.4.4 (oficial, sin dependencia JS, sin cambios a `capabilities/*.json` porque no expone comandos invoke) como el **primer** plugin registrado — confirmado leyendo el propio código fuente de `tauri` 2.11.5 que los plugins se inicializan síncronamente, en orden de registro, antes del `.setup()` que abre el vault y corre `run_startup_recovery`/`sweep_stale_temp_files`; las tres implementaciones de plataforma del plugin llaman a `std::process::exit(0)` dentro de su propio setup al detectar una segunda instancia, así que ese código nunca se ejecuta dos veces. Cuaderno Clínico pasa a ser explícitamente single-instance; el callback solo enfoca la ventana "main" existente. Verificación real en Linux bajo Xvfb quedó bloqueada porque este entorno de desarrollo no tiene un bus de sesión D-Bus activo — limitación del entorno, documentada explícitamente, pendiente de validación en un escritorio Linux real junto con macOS/Windows. **CRYPTO-1**: hasta esta fase, la misma DEK del vault se usaba tanto como clave raw de SQLCipher como clave AES-256-GCM para envolver la DEK de cada documento, sin separación de dominio. `SCHEMA_V10` (aditiva) agrega `documents.key_wrap_version` (`1` = legacy sin cambios, `2` = subclave derivada vía HKDF-SHA256 con el crate `hkdf` 0.12.4 — RustCrypto, compatible con el `digest 0.10` que ya usa `sha2` sin duplicar esa dependencia); `wrap_file_key` produce exclusivamente `key_wrap_version = 2` desde esta fase, `unwrap_file_key` despacha exclusivamente según la columna de cada fila, nunca autodetección ni fallback silencioso. **Sin ninguna migración automática, eager, lazy ni en background de v1 a v2** — decisión explícita tras comparar tres estrategias (`Plan-Fase-17...md` §6-9): los documentos legacy de Fase 16 permanecen deliberadamente en `key_wrap_version = 1`, riesgo/deuda técnica residual conocida y aceptada, nunca "una migración pendiente". Un fixture legacy congelado (bytes calculados una sola vez con un programa `aes-gcm` independiente, nunca con el código de este módulo) prueba que el algoritmo de Fase 16 sigue siendo legible indefinidamente. 791/791 tests Rust en verde (766 previos + 25 nuevos: 6 en `db::migrations` para `V10`, 2 nuevos en `backup::archive` para BACKUP-1, 12 en `security::session` para HKDF/`KeyWrapVersion`, 2 en `repositories::documents`, 5 en `services::documents`, 1 mixto v1/v2 en `backup::service`) — 3 suites completas consecutivas idénticas, `cargo clippy --release --all-targets` sin advertencias, build limpio, `npm run build`/`npm run lint` limpios (25 warnings preexistentes, 0 nuevos, 0 errores). Dos dependencias nuevas, ambas explícitamente aprobadas y verificadas por compatibilidad real (no por versión propuesta a priori): `hkdf` 0.12.4 y `tauri-plugin-single-instance` 2.4.4 — ninguna forzó una actualización de `tauri`, `sha2` ni ninguna otra dependencia existente (confirmado por diff de `Cargo.lock`: solo adiciones, cero paquetes existentes cambiando de versión). Detalle completo en `docs/documents.md` (§4.1) y `docs/backup-restore.md` |
| **Continuación post-Fase 19** — Rediseño del Plan de Seguridad en 6 pasos (Stanley & Brown) | ✅ Completada | A partir de validación real en Windows (19/09/2026). `SCHEMA_V11` (aditiva + un rebuild de tabla sin transformar ni un byte, mismo patrón que `SCHEMA_V9`): tabla nueva `safety_plan_list_items` para las tres listas agregables (Paso 1 señales, Paso 2 estrategias, lugares del Paso 3 — nunca un textarea único, cada ítem es su propia fila con persistencia inmediata) y `safety_plan_contacts` ampliada (`contact_type` +`distraction_person`/`help_contact`, +`address`/`service_phone`/`is_emergency_contact`/`is_crisis_service` para el Paso 5). `support_person` deja de ser creable pero sigue siendo 100% legible/editable en planes que ya lo tenían — nunca reclasificado automáticamente. Contenido narrativo heredado (`warning_signs`/`internal_strategies`/`social_support_strategies`/`crisis_steps`/`notes`) se conserva íntegro y se muestra en paneles "heredado" para planes anteriores al rediseño; un plan nuevo nunca puebla esos campos. `create_draft_from_current` ahora también copia los ítems de lista al actualizar plan. 65 tests de `safety_plans` (18 nuevos), suite completa 811/811, `cargo clippy` limpio. Frontend reescrito en seis secciones explícitas (`SafetyPlanTab.tsx`). **Limitación de validación conocida**: sin prueba GUI interactiva completa en este entorno (Xvfb sin compositor real, eventos sintéticos de scroll/Tab no fiables) — solo se verificó arranque/creación de vault/navegación básica; validación GUI completa pendiente para Windows. Detalle completo en `docs/safety-plan.md` §20 |
| **Continuación post-Fase 19** — Biblioteca global de recursos (`library_resources`/`library_resource_patients`) | ✅ Completada | Feature nueva, distinta de Documentos por paciente: un recurso se sube una sola vez y se asocia a cualquier número de pacientes sin duplicar nunca el archivo físico ni su cifrado. Reutiliza `library_resources` (declarada desde `SCHEMA_V1` sin código que la usara — ya apuntaba a `documents.file_document_id`, previendo exactamente esta reutilización): el archivo de un recurso, si tiene uno, es una fila de `documents` con `patient_id = NULL` (columna nullable desde `SCHEMA_V1`; `NewDocumentRow.patient_id` pasa de `&str` a `Option<&str>`, cambio mínimo y retrocompatible — Documentos por paciente sigue pasando siempre `Some(...)`). Cero tabla de archivos nueva, cero implementación de cifrado nueva: reutiliza directamente `document_crypto::*` y las utilidades de `services::documents`. `SCHEMA_V12` (aditiva) agrega la única pieza que faltaba: `library_resource_patients`, la relación N:M con pacientes. "Eliminar" un recurso es un archivado reversible (nunca borrado físico): si sigue asociado a pacientes, se rechaza con un conteo a menos que se confirme explícitamente (`force=true`) — las asociaciones nunca se tocan, con o sin confirmación. Vincular a un paciente archivado se rechaza; desvincular y consultar siempre permitidos. Backup/Restore sin ningún cambio de código: `list_all_storage_paths` nunca filtró por `patient_id`, así que el archivo de un recurso de Biblioteca queda incluido automáticamente — verificado con 2 tests dedicados (inclusión en manifiesto + restauración completa con su asociación). 841/841 tests Rust en verde (73 nuevos: 6 de migración, 12 de repositorio, 13 de servicio, 2 de backup/restore, resto vía compilación de comandos), `cargo clippy` limpio. Pantalla global nueva "Biblioteca" (`/library`, nav principal) y sección "Biblioteca" en la ficha del paciente (`src/features/library/`), `npm run build`/`npm run lint` limpios. Detalle completo en `docs/library.md` |
| **FASE 1** — Causa raíz WIN-DB-01/WIN-DB-02, validada en Windows real | ✅ Completada | `security::vault_manager::unlock_vault`/`recover_access` — las dos únicas rutas reales de reapertura de un vault existente — autenticaban y abrían SQLCipher pero nunca corrían `db::run_migrations` (solo `PendingVaultCreation::finalize`, exclusivo de un vault **nuevo**, migraba). Un vault creado bajo un esquema anterior quedaba congelado en ese `PRAGMA user_version` para siempre en cada desbloqueo normal, sin importar cuántas veces se actualizara el binario — causa raíz única de que Plan de Seguridad (`SCHEMA_V11`) y Biblioteca↔paciente (`SCHEMA_V12`) fallaran sobre un vault real preexistente pese a pasar 841/841 tests: todo helper de test llama a `run_migrations` directamente, nunca a través del flujo real de desbloqueo. Corrección: ambas funciones migran siempre tras autenticar; se separó "autenticar+abrir" de "migrar" en dos funciones internas (`*_without_migrating`) porque esto rompía una regresión real en `backup::service::restore_backup`, que necesita leer el `user_version` crudo del backup staged **antes** de migrarlo para poder rechazar un backup de una versión futura de la app. Dos tests de regresión reproducen el escenario real completo (vault V10 con datos reales → función real de desbloqueo → V12 → los servicios que Windows reportó rotos funcionan). Además, varias pantallas quedaban en "Cargando…" eterno tras un error de carga en vez de mostrarlo — corregido con reintentar en Plan de Seguridad y Biblioteca del paciente. **Validado en Windows real sobre el mismo vault preexistente que originó el reporte**: Plan de Seguridad y Biblioteca↔paciente pasan a funcionar correctamente. 843/843 tests Rust en verde, `cargo clippy` limpio. Detalle completo en `docs/safety-plan.md` §21 y `docs/library.md` §12. |
| **FASE 2/3/4** — Hard delete (Biblioteca/Pacientes), exportar Plan de Seguridad, UX (estadísticas/Timeline/Calendar) | ✅ Completada | Autorizada explícitamente tras la validación de FASE 1. **FASE 2A**: borrado físico e irreversible de un recurso de Biblioteca, solo desde "Archivados", exige cero asociaciones con pacientes (sin `force`, a diferencia de archivar) — transacción SQL manual que borra la fila de `documents` (si tiene archivo) y la de `library_resources`, ciphertext borrado del disco *después* de confirmar (best-effort). **FASE 2B**: borrado físico e irreversible de un paciente ya archivado — auditoría completa de las 14 tablas relacionadas (directas y transitivas) del esquema real, borrado explícito tabla por tabla en orden hojas→raíz (nunca depende de los `ON DELETE CASCADE` ya declarados, instrucción explícita), una única transacción SQL, resumen de alcance de solo lectura antes de confirmar, advertencia neutral de conservación, y `library_resources`/`library_resource_patients` con la misma regla que 2A (solo se borra la relación, nunca el recurso global) — detalle completo con la tabla de auditoría en `docs/patients-vertical.md`. **FASE 3**: "Exportar plan" en el Plan de Seguridad vigente usa `window.print()` nativo del WebView (Guardar-como-PDF ya es un destino estándar del diálogo de impresión en Windows/macOS) en vez de una dependencia Rust de generación de PDF — **cero dependencias nuevas**; portal `#print-root` hermano de `#root`, oculto en pantalla y único contenido visible bajo `@media print`; nombre del paciente opcional; nunca incluye diagnóstico/notas/UUIDs/historial — ver `docs/safety-plan.md` §22. **FASE 4**: paleta categórica de ocho tonos realmente distintos (antes derivada del único acento verde) para Estadísticas, validada con la herramienta de accesibilidad de paletas del proyecto (`docs/geographic-stats.md`); pestaña "Línea temporal" oculta de la navegación (nunca un placeholder "Próximamente" visible en una RC, estructura conservada para una implementación futura); Ajustes → Google Calendar reorganizado con una vista simple por defecto y el formulario de Client ID/Client Secret detrás de "Configuración avanzada" — sin ningún cambio al modelo OAuth (`docs/google-calendar.md`); auditoría de loading/error en las superficies nuevas de esta fase (reintentar donde correspondía). 852/852 tests Rust en verde (9 nuevos: 6 de hard-delete de Biblioteca, 3 de hard-delete de pacientes — incluido un test que siembra una fila en cada una de las 14 tablas auditadas), `cargo clippy --release --all-targets` sin advertencias, build/lint frontend limpios sin categoría nueva de warning, cero dependencias nuevas. Detalle completo en `docs/patients-vertical.md`, `docs/library.md` §13-14, `docs/safety-plan.md` §22, `docs/geographic-stats.md` y `docs/google-calendar.md`. |
